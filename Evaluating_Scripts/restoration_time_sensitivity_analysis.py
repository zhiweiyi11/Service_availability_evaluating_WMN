...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：Service_availability -> restoration_time_sensitivity_analysis
@IDE    ：PyCharm
@Author ：Yi Zhiwei
@Date   ：2023/7/3 22:10
@Desc   ：恢复时间的敏感性分析
...
def save_results(origin_df, file_name):
    ...
    origin_df.to_excel(r'..\Results_Output\RestorationTime_results\{}_{}.xlsx'.format(file_name, time2), index=False)
    print('数据成功保存')
...
def draw_line_plot(x_data, y_data, file_name):
    ...
    plt.savefig(r'..\Pictures_Saved\折线图_{}.jpg'.format('{}'.format(file_name), dpi=1200) )
    plt.show()

def calculate_RestorationTime_analysis(RestorationTime_list, N, G, Apps, App_priority_list, beta_list):
    # 服务可用度的MTTR敏感性分析
    MLife = 800
    ...
    for path_calculating_time in RestorationTime_list:
        ...
        SLA_avail.loc[:, path_calculating_time] = pd.Series(current_SLA_avail) # 每一列存储该MTTF值下的业务可用度
        WHOLE_avail.loc[:, path_calculating_time] = current_whole_ave
        EACH_avail.loc[:, path_calculating_time ] = current_each_avail.apply(np.mean, axis=1) # 对每一行求平均值
...
def resource_analysis(RestorationTime_list, File_name_list):
    ...
    availability_different_demand_local = pd.DataFrame(index=RestorationTime_list) # 存储结果
    availability_different_demand_global = pd.DataFrame(index=RestorationTime_list)
